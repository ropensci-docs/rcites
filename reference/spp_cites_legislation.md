...
Retrieve current CITES appendix listings and reservations, CITES quotas,
and CITES suspensions for a given taxon concept.
...
- scope:
  ...
  values are taken among `current`, `historic` and `all`. Default is
  `current`.
...
Otherwise, an object of class `spp_cites_leg` (or `spp_cites_leg_multi`
if `length(taxon_id)>1`) is returned which is a list of three data
frames:
